...
from .booking import Booking # Import Booking model
...
class ParkingSpace(db.Model):
    ...
    is_booked = db.Column(db.Boolean, default=False, nullable=False) # Legacy field, or for admin override
    ...
    def to_dict(self):
        ...
        # Determine current booking status
        is_currently_booked = False
        booked_until = None
        now = datetime.utcnow()
        
        # Query for an active booking for this space
        active_booking = Booking.query.filter(
            Booking.space_id == self.id,
            Booking.end_time > now,
            Booking.status.in_(['confirmed', 'active']) # Ensure booking is active
        ).order_by(Booking.end_time.asc()).first()

        if active_booking:
            is_currently_booked = True
            booked_until = active_booking.end_time.isoformat()
        ...
        return {
            ...
            # "is_booked": self.is_booked, # Removed, replaced by dynamic check
            "is_currently_booked": is_currently_booked, # Dynamic booking status
            "booked_until": booked_until, # Time until which it's booked, if currently booked
            "owner_id": self.owner_id,
            "owner_details": self.owner.to_dict() if self.owner else None,
            "images": images_data,
