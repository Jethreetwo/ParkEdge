...
from flask import (
    ...
    jsonify,
    current_app,
)  # Added current_app for logger access
...
# Define the default upload folder path relative to this file
DEFAULT_UPLOAD_FOLDER = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "uploads", "parking_images"
)
# Ensure the default upload folder exists
os.makedirs(DEFAULT_UPLOAD_FOLDER, exist_ok=True)

# Backwards compatibility constant used by tests
...
def create_space():
    ...
    if price_unit not in allowed_units:
        return (
            jsonify(
                {
                    "error": f"Invalid price_unit. Allowed units are: {', '.join(allowed_units)}"
                }
            ),
            400,
        )
    ...
    # Resolve upload directory from app config if provided
    ...
    for index, file_storage in enumerate(files):
        if file_storage and file_storage.filename:
            ...
            try:
                ...
                saved_filenames_for_cleanup.append(
                    filename
    ...
    try:
        ...
        for fname in saved_filenames_for_cleanup:
            try:
                ...
                current_app.logger.error(
                    f"Error cleaning up file {fname} after db commit failure.",
                    exc_info=True,
                )
                pass
        ...
        current_app.logger.error(
            f"DATABASE COMMIT ERROR in create_space: {str(e)}", exc_info=True
        )
        traceback.print_exc()  # Also print stack trace to console/stderr for dev visibility

        return (
            jsonify(
                {"error": "Failed to create parking space due to a database error."}
            ),
            500,
        )
...
def get_spaces():
    """Return all parking spaces that are not currently booked."""
    now = datetime.utcnow()
    ...
    return jsonify([space.to_dict() for space in available_spaces]), 200
...
def book_space(space_id):
    ...
    if duration_unit not in allowed_duration_units:
        return (
            jsonify(
                {
                    "error": f"Invalid duration_unit. Allowed units are: {', '.join(allowed_duration_units)}"
                }
            ),
            400,
        )
    ...
    active_booking_count = Booking.query.filter_by(
    ...
    if active_booking_count >= MAX_CONCURRENT_BOOKINGS:
        return (
            jsonify(
                {
                    "error": f"You have reached the maximum limit of {MAX_CONCURRENT_BOOKINGS} active bookings."
                }
            ),
            403,
        )
    ...
    # Availability check using active bookings
    active_booking = Booking.query.filter(
        Booking.space_id == space.id,
    ...
    if active_booking:
        return jsonify({"error": "Parking space is already booked"}), 409
    ...
    if space.price_unit == "hour" and duration_unit == "hours":
        ...
        return (
            jsonify(
                {
                    "error": f"Price unit mismatch or unsupported calculation: space priced per {space.price_unit}, booking requested in {duration_unit}."
                }
            ),
            400,
        )
